	...
	public function HtmlShowOptionsPage() {
		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
			...
			foreach($this->sg->GetOptions() as $k=>$v) {
				...
				if(!current_user_can( 'manage_options' ) && in_array($k,array("sm_b_time","sm_b_memory","sm_b_style","sm_b_style_default"))) {
					continue;
				}
			...
			if(current_user_can( 'manage_options' )) $this->sg->SetPages($this->HtmlApplyPages());
					...
					<?php
						...
						<div style="min-height:150px;">
							<ul>
								...
								<?php if(current_user_can( 'manage_options' )) echo "<li>" . str_replace("%d",wp_nonce_url($this->sg->GetBackLink() . "&sm_rebuild=true&sm_do_debug=true",'sitemap'),__('If you encounter any problems with your sitemap you can use the <a href="%d">debug function</a> to get more information.','sitemap')) . "</li>"; ?>
							</ul>
							<ul>
								<li>
